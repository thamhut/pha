<div id="header">
	<div class="header">
		<a href="{{ this.url.get() }}">
            <img alt="<?php echo $lang->_('alt_logo') ?>" src='{{ static_url("public/img/icon_header.png") }}' class="logo"/>
        </a>
        ...
        <span>{{ lang._('local') }}</span>
        <div class="choise_city">
            {% if user['city'] is defined and user['city'] != '' %}
                {{ user['city']['name_city'] }}
            {% else %}
                {{ lang._('nationwide') }}
            {% endif %} 
        ...
        <div id="show_city" class="show_city leftmenu1">
            <div style="font-size: 13px;padding: 15px;text-align: center;"><a href="<?php echo $this->url->get('index/choise_city?url='.urlencode($this->getCurrentPageURL).'&city=0'); ?>">---{{ lang._('nationwide') }}---</a></div>
            <ul>
