<div class="box_container_new">
    ...
    <?php
        echo '<a itemprop="url" href="'.$this->url->get('category').'/'.$this->utils->converToUrl($name).'_i'.$id_cate.'" class="repadL ';
        ...
        foreach($category_data as $item)
        {
            if($id_cate == $item->id)
            {
                echo ' |<a itemprop="url" href="'.$this->url->get('category').'/'.$this->utils->converToUrl($name.'-'.$item->name).'_i'.$item->id_level.'"  class="';
    ...
    <div style="" class="fillter_box" id="news_nav_filter">
        <input type="text" hidden="" alt="{{ this.utils.converToUrl(name) }}" id="idcate" value="{{ id_cate }}" />
    ...
    <div class="box_data_new">
        <ul itemscope itemtype="http://schema.org/BreadcrumbList">
            <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="li_title">
                <div itemprop="name">{{ lang._('new_post') }}</div>
            </li>
            <?php 
            $i = 0;
            ...
            <?php 
                ...
                $i++;
            ?>
            <li onmousemove="mymousemove(this);" onmouseout="mymouseout(this);" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                <div class="div_box_img_title_info">
                    <div class="div_img"><img itemprop="image" alt="<?php echo $this->utils->enhtml($itemContent['title']); ?>" src="<?php if($this->utils->explode_first($itemContent['img'], ',')) echo $this->utils->explode_first($itemContent['img'], ','); else echo $this->url->get().'public/img/noimg.png';?>" width="90" height="90" /></div>
                    <div class="div_box_title">
                        <a itemprop="url" target="_blank" href="<?php echo $this->url->get('detail/'.$this->utils->converToUrl($itemContent['title']).'_i'.$itemContent['id'].'.html'); ?>">
                    ...
                    <h5>
                     ...
                     ?></span>
                        ...
                        <a itemprop="url" target="_blank" href="<?php echo $this->url->get('detail/'.$this->utils->converToUrl($itemContent['title']).'_i'.$itemContent['id']).'.html'; ?>">{{ lang._('continiu') }} ...</a>
                    </h5>
                </div>
            </li>
            <?php }?>
    ...
    <div class="div_box_container_right">
        ...
        <div class="div_lst_cate div_lst_news_hot">
            <ul itemscope itemtype="http://schema.org/BreadcrumbList">
                <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="li_title">{{ lang._('hot_posts') }}</li>
                <?php 
                foreach($data['content_hot'] as $icontent)
                {
                ?>
                <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" ><div style="float: left; width: 50px;"><img itemprop="image" alt="<?php echo $this->utils->enhtml($icontent->title);?>" src="<?php
                ...
                echo $img;
                ?>" style="vertical-align: middle; margin-right: 5px; max-width: 50px; max-height: 50px;"></div><div style="float: right; width: 182px; margin-top: -5px;"><a itemprop="url" title="<?php echo $this->utils->enhtml($icontent->title);?>" target="_blank" href="<?php echo $this->url->get('detail/'.$this->utils->converToUrl($icontent->title).'_i'.$icontent->id).'.html'; ?>"><?php echo $this->utils->enhtml($icontent->title);?></a></div><div class="clear"></div></li>
                <?php }?>
            </ul>
        </div>
    </div>
