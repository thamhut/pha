<?php
    ...
    function my_callback_raovatvn($element) {
        ...
        $element->outertext = '';
        if ($element->class=='detail-icon-box')
        $element->outertext = '';
        if ($element->class=='detail-action-box')
        $element->outertext = '';
        if ($element->class=='h_10')
        $element->outertext = '';
        if ($element->tag=='ins')
            $element->outertext = '';
        if ($element->tag=='img')
    ...
    {
        $link = read('/var/www/html/chotam/runlink/loadlink.txt');
    ...
    function loadcontent_nhatro($url)
    {
        $data = get_fcontentByGoogle($url);
        $html = str_get_html($data);
        $result = array();
        foreach($html->find('div.ptdt') as $e)
        {
        ...
        foreach($html->find('div.ptdl img') as $e)
        {
           $result['img'][] = 'http://thuephongtro.com'.$e->src;
        }
        
        foreach($html->find('div.chitiet') as $e)
        {
            $e = preg_replace('#<div(.*?)>#i', '', $e);
           $e = str_replace('</div>', '', $e);
           $result['content'] = $e;
    ...
    function loadcontent_raovatvn($url)
    {
        ...
        foreach($html->find('div#detailItem div.dIconBox span') as $e)
        {
            if($e->title == 'Tỉnh thành')
            {
                $city = explode('Khu vực: ', $e);
                $city = trim($city[1]);
                $result['city'] = getCity(strip_tags($city));
                break;
            }
        ...
        foreach($html->find('div#detailItem div.dIconBox a') as $e)
        {
            if(strpos($e->href,'@')!=false)
            {
                $result['lienhe'][1] = trim(strip_tags($e));
                break;
            }
        }
        $html->set_callback('my_callback_raovatvn');
        
        foreach($html->find('div#detailItem div div') as $e)
        {
            if($e->class =='' && strlen($e)>200)
            {
                $e = preg_replace('#<div(.*?)>#i', '', $e);
                $e = str_replace('</div>', '', $e);
        ...
        $i=0;
        foreach($html->find('div#detailItem div div img') as $e)
        {
            if($i<5)
            {
                $result['img'][] = $e->src;
    ...
    function loadcontent($url, $cate)
    {
        ...
        if(strpos($url,'raovat.vn')==11)
        {
            foreach($html->find('td.titleListDetail div.title a') as $e)
            {
                ...
                if($check[0]==0)
                {
                    ...
                    if($title!=''&& $content!='' && strlen($content)>400){
        ...
        if(strpos($url,'raovat.com')==7)
        {
            ...
            foreach($html->find('div.tinrao_list ul li a') as $e)
            {
               if($i%2==0)
               {
                    $check = checkLink("http://raovat.com/".$e->href);
                    if($check[0]==0)
                    {
            ...
            foreach($html->find('div#itemListContent div.cropit a') as $e)
            {
                if($e)
                {
                    if(checkLink($e->href)!=0)
                    {
                        ...
                        if($title!=''&& $content!='' && strlen($content)>100){
                            insertdb($url, $e->href, $date, 'jaovat.com', $cate, addslashes($content), 2, addslashes($title), $img, $city, $lienlac);
                        }
                    }
                }
            }
        ...
        if(strpos($url,'thuephongtro.com')==7)
        {
            foreach($html->find('span#ctl00_ContentPlaceHolder1_dtlPm a') as $e)
            {
               if($e)
                {
                    if(checkLink('http://thuephongtro.com'.$e->href)!=0)
                    {
                        $data2 = loadcontent_nhatro('http://thuephongtro.com'.$e->href) ;
                        $title = isset($data2['title'])?$data2['title']:'';
                        $city = 30;
                        $img = '';
                        if(isset($data2['img']))
    ...
    function checkLink($url)
    {
        ...
        $iconn = mysqli_connect('localhost', 'root', 'trietquyendao8', 'chotam_raovat') or die('Eo ket noi duoc') ;
    ...
    function insertdb($url, $url_chil, $date, $source, $cate, $content, $uid, $title, $img, $address, $lienlac)
    {
		...
		$iconn = mysqli_connect('localhost', 'root', 'trietquyendao8', 'chotam_raovat') ;
