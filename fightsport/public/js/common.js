...
function show_right_box(type, e)
{
    $(".div_top_music_box ul li").removeClass('current');
    $(e).addClass('current');
    $(".box_item_right_menu").hide();
    $('div#'+type+'_box_right').show();
}

function show_hot(type, e)
{
    $(".fillter_date a").css('text-decoration','none');
    $(e).css('text-decoration','underline');
    $(".hot_box").hide();
    $('div#'+type+'_hot_box').show();
}

function change_img()
{
    $("#add_avata").html('<input style="margin: 10px 0;" id="file" class="wtd" type="file" name="avata[]" placeholder="chọn đường dẫn" />');
}
...
function insert_nhacsan()
{
    ...
    var url = 'http://fightsportgroup.com/runlink/insert.php';
    if(nhacdj!='')
    {
        $.ajax({
            type: 'GET',
